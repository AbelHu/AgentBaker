...
function Upload-GuestVMLogs {
    ...
    try {
        if ($ExitCode -ne 0) {
            ...
            Create-Directory -Path $aksLogFolder

            Write-Log "Creating SymbolicLink for C:\AzureData\CustomDataSetupScript.log in $aksLogFolder"
            New-Item -ItemType SymbolicLink -Path (Join-Path $aksLogFolder "CustomDataSetupScript.log") -Target "C:\AzureData\CustomDataSetupScript.log"

            # Create a work folder
            Write-Log "Creating $tempWorkFoler"
            Create-Directory -Path $tempWorkFoler
            cd $tempWorkFoler
