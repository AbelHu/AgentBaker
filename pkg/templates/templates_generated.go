...
finally
{
    ...
    if ($global:ExitCode -ne 0) {
        Write-Error "ExitCode: `+"`"+`"$global:ExitCode`+"`"+`", Output: `+"`"+`"$($global:ErrorCodeNames[$global:ExitCode])`+"`"+`", Error: `+"`"+`"$global:ErrorMessage`+"`"+`", ExecDuration: `+"`"+`"$ExecutionDuration`+"`"+`""
        exit $global:ExitCode
    } else {
        Write-Error 'This is a test. It succeeded.'
        exit 0
    }
}
`)
